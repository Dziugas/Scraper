...
outputFile = open('accomodation.csv', 'w', newline='', encoding='utf-8')
outputWriter = csv.writer(outputFile)

# Create a list with column titles and write it to the csv as the first line
column_names = ['ID', 'Source', 'Title', 'Address', 'Phone', 'Email', 'Website', 'Working Hours', 'Description', 'Booking.com', 'TripAdvisor', 'Category']
outputWriter.writerow(column_names)

# empty list for adding a new line of data to write to csv
empty_list = []

# css selectors for the elements that we need to extract text from
...
description_selector = '.typography'
# css selectors for the elements that we need to extract links from
tripadvisor_selector = '.cdsComponent a'

bookingcom_selector = '.button-green'
#list with all the css selectors
css_selectors_text = [title_selector, address_selector, phone_selector, email_selector, website_selector, working_hours_selector, description_selector]
css_selectors_href = [bookingcom_selector, tripadvisor_selector]
...
driver.get('http://visit.kaunas.lt/en/accommodation/')
...
for link, category in dictionary.items():
    ...
    for object in css_selectors_text:
    ...
    for object in css_selectors_href:
        elements_raw = dom.select(object)
        elements = [element.get('href') for element in elements_raw]
        if len(elements)>0:
            empty_list.append(elements[0])
        else:
            empty_list.append('')
